<?php
  ...
  if($con){
    $sno=$_GET["sno"];
    $name=$_GET["name"];
    $sql="SELECT * from income WHERE sno='$sno'";
    $que=$con->query($sql);
    if($que->num_rows>0){
...
<body>
    <div class="container d-flex flex-column justify-content-center align-items-center mt-4">
        <h2 class="text-success"> Income Tracker</h2>

        <div class="main">
        <form action="upinc.php" method="get">
        ...
        <input type="hidden" name="name" id="name" value="<?php echo $name; ?>">
            ...
            <button type="submit"  class="btn btn-success mt-3 btn1">Update Income</button>
        </form>
        </div>
