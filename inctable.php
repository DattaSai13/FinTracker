...
<div class="container ">
        <h2 class="text-success text-center"> Incomes</h2>
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                $con = new mysqli("localhost", "root", "", "expense_tracker");
                $name=$_GET["name"];
                $sql = "SELECT * FROM income WHERE Names='$name' ORDER BY date DESC";
                $que = $con->query($sql);
                
                $sql_total_income = "SELECT SUM(Amount) AS total_income FROM income WHERE Names='$name' ";
                $total_income = $con->query($sql_total_income)->fetch_assoc()["total_income"];
                if ($que->num_rows > 0) {
                    while ($row = $que->fetch_assoc()) {
                        ...
                        echo "<td><a class='text-info' href='edinc.php?sno={$row['Sno']}&name={$row['Names']}'> <i class='fa-solid fa-pen-to-square'></i> </a></td>";
                        echo "<td><a class='text-danger' href='delinc.php?sno={$row['Sno']}&name={$row['Names']}' onclick='return dele()' ><i class='fa-solid fa-trash'></i></a></td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='5' class='text-center'>No income records found</td></tr>";
                }
                echo "<tr align='center'><td colspan='3'><b>Total Income: ₹$total_income</b></td></tr>";
                ?>
            </tbody>
        </table>
    ...
    <script>
        function dele(){
            return confirm("Are your sure you want to delete this source?")
        }
    </script>
